...
#-----------------------------------------
catch_file  <-  "Katia_Catch.csv"  # "O_Stocks_Catch_MainlandIndia.csv" #    name of file containing "Stock", "yr", "ct", and optional "bt"
id_file     <-  "Katia_ID.csv"  # "O_Stocks_ID_MainlandIndia.csv"  #   name of file containing stock-specific info and settings for the analysis
...
for(stock in stocks) {
  ...
  # ---------------------------------------------------------------------
    ...
    if(is.na(q.start)==F & is.na(q.end)==F) {
      ...
      if(is.na(bt[nyr])==T | is.na(bt[nyr-lyr])==T) {
        cat("In ID file, set years to use for estimating q (q.start & q.end)")
        break() }
