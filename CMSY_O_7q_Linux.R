...
#-----------------------------------------
catch_file  <-  "O_Stocks_Catch_15_Med.csv" #  name of file containing "stock", "yr", "ct", and optional "bt" ##modification for as-a-service
id_file     <-  "O_Stocks_ID_18_Med.csv"  #  name of file containing stock-specific info and settings for the analysis ##modification for as-a-service

outfile     <- paste("Out_",format(Sys.Date(),format="%B%d%Y_"),id_file,sep="") # default name for output file
outfile.txt <<- paste(outfile,".txt", sep="")  ##modification for as-a-service

#----------------------------------------
# Select stock to be analyzed
#----------------------------------------
#stocks      <-NA ##modification for as-a-service
...
stocks <-  "SARDPIL_SA_EXAMPLE" # c("SEPIOFF_CY","MICRPOU_IS","EPINGUA_IS","CHAMGAL_SA","CORYHIP_SA","ILLECOI_SA") ##modification for as-a-service
